	...
	function search($search, $limit=20,$offset=0,$column="giftcard_number",$orderby='asc')
	{
		...
		$this->db->join('customers','people.person_id = customers.person_id', 'left');
 		...
 		{			
			if($this->config->item('supports_full_text') && !$this->config->item('legacy_search_method'))
			{
 				$this->db->where("(MATCH (`description`, giftcard_number) AGAINST (".$this->db->escape(q($search).'*')." IN BOOLEAN MODE".") or MATCH(".$this->db->dbprefix('people').".first_name) AGAINST (".$this->db->escape(escape_full_text_boolean_search($search).'*')." IN BOOLEAN MODE".") or MATCH(".$this->db->dbprefix('people').".last_name) AGAINST (".$this->db->escape(escape_full_text_boolean_search($search).'*')." IN BOOLEAN MODE".")  or MATCH(".$this->db->dbprefix('people').".first_name, ".$this->db->dbprefix('people').".last_name) AGAINST (".$this->db->escape(escape_full_text_boolean_search($search).'*')." IN BOOLEAN MODE".")) and ".$this->db->dbprefix('giftcards').".deleted=0", NULL, FALSE);			
			}
			else
			{
				$this->db->where("(first_name LIKE '%".$this->db->escape_like_str($search)."%' or 
