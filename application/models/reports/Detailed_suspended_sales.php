...
class Detailed_suspended_sales extends Report
{
	...
	public function getData()
	{
		...
		$this->db->select('customer_data.account_number as account_number,locations.name as location_name, sales_items_temp.sale_id, register_name, sales_items_temp.sale_time, sales_items_temp.sale_date, sum(actual_quantity_purchased) as items_purchased, CONCAT(sold_by_employee.first_name," ",sold_by_employee.last_name) as sold_by_employee, CONCAT(employee.first_name," ",employee.last_name) as employee_name, customer.person_id as customer_id, CONCAT(customer.first_name," ",customer.last_name) as customer_name, sum(subtotal) as subtotal, sum(total) as total, sum(tax) as tax, sum(profit) as profit, sales_items_temp.payment_type, sales_items_temp.comment, sales.suspended, sales.was_layaway, sales.was_estimate', false);
		...
		$this->db->select('sale_id, sale_time, sale_date, item_number, items.product_id as item_product_id,item_kits.product_id as item_kit_product_id, item_kit_number, items.name as item_name, item_kits.name as item_kit_name, sales_items_temp.category,sales_items_temp.category_id, quantity_purchased, serialnumber, sales_items_temp.description, subtotal,total, tax, profit, discount_percent, items.size, ', false);
		$this->db->from('sales_items_temp');
