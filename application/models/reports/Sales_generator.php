...
class Sales_generator extends Report
{
	...
	public function getData()
	{
		...
		{
			$this->db->select('customer_data.account_number as account_number, locations.name as location_name,sale_id, sale_time, register_name, sale_date, sum(actual_quantity_purchased) as items_purchased, CONCAT(sold_by_employee.first_name," ",sold_by_employee.last_name) as sold_by_employee, CONCAT(employee.first_name," ",employee.last_name) as employee_name, CONCAT(customer.first_name," ",customer.last_name) as customer_name, sum(subtotal) as subtotal, sum(total) as total, sum(tax) as tax, sum(profit) as profit, payment_type, comment', false);
			...
			$this->db->select('sales_items_temp.category_id,items.unit_price as current_selling_price, sale_id, items.item_id, item_kits.item_kit_id, sale_time, sale_date, item_number, items.product_id as item_product_id,item_kits.product_id as item_kit_product_id, item_kit_number, items.name as item_name, item_kits.name as item_kit_name, sales_items_temp.category, sales_items_temp.sale_item_temp_cost_price, quantity_purchased, quantity_purchased as items_purchased, serialnumber, sales_items_temp.description, subtotal,total, tax, profit, discount_percent, items.size', false);
			$this->db->from('sales_items_temp');
		...
		else
		{
			...
			$this->db->select('customer_data.account_number as account_number,locations.name as location_name,sale_id, sale_time, sale_date, register_name, sum(actual_quantity_purchased) as items_purchased, CONCAT(sold_by_employee.first_name," ",sold_by_employee.last_name) as sold_by_employee, CONCAT(employee.first_name," ",employee.last_name) as employee_name, CONCAT(customer.first_name," ",customer.last_name) as customer_name, sum(subtotal) as subtotal, sum(total) as total, sum(tax) as tax, sum(profit) as profit, payment_type, comment', false);
			$this->db->from('sales_items_temp');
			$this->db->join('locations', 'sales_items_temp.location_id = locations.location_id');
			$this->db->join('people as employee', 'sales_items_temp.employee_id = employee.person_id');
			$this->db->join('people as sold_by_employee', 'sales_items_temp.sold_by_employee_id = sold_by_employee.person_id', 'left');
			...
			$this->db->select('sales_items_temp.category_id, items.unit_price as current_selling_price, sale_id, items.item_id, item_kits.item_kit_id, sale_time, sale_date, item_number, items.product_id as item_product_id,item_kits.product_id as item_kit_product_id, item_kit_number, items.name as item_name, item_kits.name as item_kit_name, sales_items_temp.category, sales_items_temp.sale_item_temp_cost_price, quantity_purchased, serialnumber, sales_items_temp.description, subtotal,total, tax, profit, discount_percent, items.size', false);
			$this->db->from('sales_items_temp');
			$this->db->join('items', 'sales_items_temp.item_id = items.item_id', 'left');
			$this->db->join('item_kits', 'sales_items_temp.item_kit_id = item_kits.item_kit_id', 'left');		
	...
	public function getSummaryData()
	{
		if ($this->params['matched_items_only'])
		{
			$this->db->select('sales_items_temp.sale_id, sum(subtotal) as subtotal, sum(total) as total, sum(tax) as tax, sum(profit) as profit,sum(actual_quantity_purchased) as items_purchased', false);
			$this->db->from('sales_items_temp');
			$this->db->where('sales_items_temp.deleted', 0);
	...
	{
		$this->db->select('sale_id, sum(actual_quantity_purchased) as items_purchased, sum(total) as total', false);
		$this->db->from('sales_items_temp');
		$this->_searchSalesQueryParams();
		$this->db->where('sales_items_temp.deleted', 0);
		$this->db->group_by('sale_id');
		...
		if ($this->params['values'][0]['f'] != 0) 
		{
			foreach ($this->params['values'] as $w => $d) 
			{
				...
				{ 
					...
					{
						$this->db->having('ROUND(total,'.$decimals.') '.str_replace("xx", join(", ", $d['i']), $ops));				
					}
					elseif($this->params['matchType'] == 'matchType_Or')
					{
						$this->db->or_having('ROUND(total,'.$decimals.') '.str_replace("xx", join(", ", $d['i']), $ops));				
					}
				}
				elseif($d['f'] == 11)
				{
