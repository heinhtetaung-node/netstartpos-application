...
class Login extends MY_Controller 
{
    	   ...
    	   $this->form_validation->set_error_delimiters('<div class="error">', '</div>');
			if($this->form_validation->run() == FALSE)
			{
				...
				if (isset($db['site']))
				{
					...
					if ($this->_is_subscription_cancelled($site_db) || $this->_is_subscription_failed($site_db) || $this->_is_in_trial($site_db))
					{
						$data['cloud_customer_info'] = $this->_get_cloud_customer_info($site_db);
						
						if ($this->_is_in_trial($site_db))
						{
							if ($this->_is_trial_over($site_db))
							{
						...
						elseif ($this->_is_subscription_failed($site_db))
						{
	...
	function _get_cloud_customer_info($site_db)
	{
	...
	function _is_trial_over($site_db)
	{
		$phppos_client_name = substr($_SERVER['HTTP_HOST'], 0, strpos($_SERVER['HTTP_HOST'], '.'));
		$site_db->select('subscr_status');	
		$site_db->from('subscriptions');	
		...
		if($this->input->post('username_or_email'))
		{
			...
			if ($employee)
			{
			   ...
			   $data['reset_key'] = $this->generate_reset_key($employee->person_id);
				...
				$this->email->from('no-reply@mg.apps.amztechnology.com/netstartposv2', $this->config->item('company'));
