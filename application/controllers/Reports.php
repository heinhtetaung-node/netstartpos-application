...
{	
	...
	function summary_categories($start_date, $end_date,$do_compare, $compare_start_date, $compare_end_date, $sale_type, $export_excel=0, $offset = 0)
	{
		...
		if ($do_compare)
		{
			...
			{
				$compare_to_categories[] = $report_data[$k]['category_id'];
			}
		...
		foreach($report_data as $row)
		{
			...
			{
				$index_compare = -1;
				$category_to_compare = $row['category_id'];
				
				for($k=0;$k<count($report_data_compare);$k++)
				{
					if ($report_data_compare[$k]['category_id'] == $category_to_compare)
					{
						$index_compare = $k;
						break;
					}
	...
	function graphical_summary_sales_graph($start_date, $end_date, $sale_type)
	{
		...
		$data = array(
			"title" => lang('reports_sales_summary_report'),
			"data" => $graph_data,
			"tooltip_template" => "<%=label %>: $currency_symbol<%= parseFloat(Math.round(value * 100) / 100).toFixed(".$this->decimals.") %>",
		);
	...
	function graphical_summary_items_graph($start_date, $end_date, $supplier_id = -1, $category_id = -1, $sale_type = 'all')
	{
		...
		$data = array(
			...
			"data" => $graph_data,
			"tooltip_template" => "<%=label %>: $currency_symbol<%= parseFloat(Math.round(value * 100) / 100).toFixed(".$this->decimals.") %>",
		   "legend_template" => "<ul class=\"<%=name.toLowerCase()%>-legend\"><% for (var i=0; i<segments.length; i++){%><li><span style=\"background-color:<%=segments[i].fillColor%>\"></span><%if(segments[i].label){%><%=segments[i].label%> ($currency_symbol<%=parseFloat(Math.round(segments[i].value * 100) / 100).toFixed(".$this->decimals.")%>)<%}%></li><%}%></ul>"			
		);
   ...
   function inventory_input()
	...
	function closeout($date, $export_excel=0)
	{
		$this->load->model('Category');
		...
		$this->load->model('reports/Summary_payments');
		$model = $this->Summary_payments;
		$model->setParams(array('start_date'=>$date, 'end_date'=>$date.' 23:59:59', 'sale_type' => 'all', 'offset'=> 0, 'export_excel' => 0));
		$sale_ids = $model->get_sale_ids_for_payments();
		$this->Sale->create_sales_items_temp_table(array('sale_ids' => $sale_ids, 'sale_type' => 'all'));
		...
		$model = $this->Closeout;
		$model->setParams(array('date'=>$date, 'sales_total_for_payments' => $sales_totals_for_payments,'export_excel' => $export_excel));

		$this->Sale->create_sales_items_temp_table(array('start_date'=>$date, 'end_date'=>$date.' 23:59:59'));
		$this->Receiving->create_receivings_items_temp_table(array('start_date'=>$date, 'end_date'=>$date.' 23:59:59'));
		...
		foreach($report_data as $row)
		{
			...
			$data_row[] = array('data'=>$row[0], 'align'=> 'left');
			$data_row[] = array('data'=>$row[1], 'align'=> 'left');
		
			$tabular_data[] = $data_row;
		}
		$data = array(
			"title" => lang('reports_closeout'),
			"subtitle" => date(get_date_format(), strtotime($date)),
			"headers" => $model->getDataColumns(),
			"data" => $tabular_data,
			"summary_data" => $model->getSummaryData(),
			"export_excel" => $export_excel,
