...
class Giftcards extends Secure_area implements Idata_controller
{
	...
	function excel()
	{
		...
		force_download('giftcards_import.'.($this->config->item('spreadsheet_format') == 'XLSX' ? 'xlsx' : 'csv'), $content);
	}

	function _excel_get_header_row()
	{
        $this->lang->load('customers');
		return array(lang('giftcards_giftcard_number'),lang('customers_account_number') . ' / ' . lang('customers_customer_id'),lang('common_description'),lang('giftcards_card_value'), lang('common_inactive'));
	}
	...
	function excel_export() {
		...
		foreach ($data as $r) {
			...
			$row = array(
				$r->giftcard_number,
                $r->account_number ? $r->account_number : $r->customer_id,
                $r->description,
	...
	function do_excel_import()
	{
		...
		else
		{
			if (($handle = fopen($_FILES['file_path']['tmp_name'], "r")) !== FALSE)
			{
				...
				{
					$giftcard_number = $sheet->getCellByColumnAndRow(0, $k)->getValue();
					if (!$giftcard_number)
					{
						$giftcard_number = '';
					}
                    $customer_id_excel = $sheet->getCellByColumnAndRow(1, $k)->getValue();
                    
                    if (!$customer_id = $this->Customer->customer_id_from_account_number($customer_id_excel))
                    {
                        if ($this->Customer->exists($customer_id_excel))
                        {
                            $customer_id = $customer_id_excel;
                        }
                        else
                        {
                            $customer_id = NULL;
					...
					$value = $sheet->getCellByColumnAndRow(3, $k)->getValue();
					...
					$inactive = $sheet->getCellByColumnAndRow(4, $k)->getValue();
