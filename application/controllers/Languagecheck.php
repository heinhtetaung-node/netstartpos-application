...
class Languagecheck extends MY_Controller {
	...
	function index($disable_white_list = 0, $fix_missing_keys = 0, $fix_matching_english_keys = 0)
	{
		...
		$this->output('h1', 'NetstartPOS - Language file checking and validation');
								...
								elseif($lang[$key] == $lang_ref[$key])
								{
									if (!$disable_white_list)
									{
										$white_list = array(
											...
											'expenses_basic_information','items_view_count','items_date','items_non_taxable','items_import','items_are_you_sure_cancel','items_unit_price_value','items_promo_price_value','items_scan_and_set',
											...
											'reports_subtotal','sales_unredeem','sales_type','sales_entry_method','sales_undeleted_voided','common_item','common_item_number_expanded','item_kits_item','reports_balance_to_pay'
										);
									}
									else
									{
