...
class Config extends Secure_area 
{
	...
	function save()
	{
		...
		if (is_on_demo_host())
		{
			...
			$batch_save_data['company_logo'] = 0;
			$batch_save_data['company'] = 'Amazing Technologies';
	...
	function do_mysqldump_backup()
	{
		...
		// 3rd: Default paths
		$mysqldump_paths[] = 'C:\Program Files\NetstartPOS Stack\mysql\bin\mysqldump.exe';  //Windows
		...
		foreach($mysqldump_paths as $mysqldump)
		{
			...
			if (is_executable($mysqldump))
			{
				...
				{
					$backup_command = "\"$mysqldump\" --host=$db_hostname --user=$db_username --password=$db_password --tz-utc=false --port=3307 --set-gtid-purged=OFF $database";				
				}
				else
				{
