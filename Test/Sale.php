...
class Sale extends CI_Model
{
	...
	{
		$customize_points=0; //Giftcard_sell_with_points HeinHtetAung
		...
		// HeinHtetAung edited for sales_quote ({
		if ($this->session->userdata('sales_quote')!="")
		{
			$this->load->library('sale_lib');
			$this->sale_lib->clear_all();
			$sale_quote=$this->session->userdata('sales_quote');
			//$this->session->unset_userdata('sales_quote');
			return $sale_quote;
		}	
		// });
		...
		$sales_data = array(
			...
			//today edit sell_with_USD ({
			'currency' => ($this->session->userdata('currency')!="")? $this->session->userdata('currency') : "",
			'exchange_rate' => ($this->session->userdata('exchange_rate')!="")? $this->session->userdata('exchange_rate') : 0,
			//});
		);
		   ...
		   $customer_info = $this->Customer->get_info($customer_id);
			...
			else
			{
	         ...
	         list($spend_amount_for_points, $points_to_earn) = explode(":",$this->config->item('spend_to_point_ratio'),2);
				...
				////Giftcard_sell_with_points HeinHtetAung ({
				foreach($payments as $payment_id=>$payment)
				{
					if(isset($payment['customize_points'])){
						$customize_points=$payment['customize_points']; 
						
					}
				}
				if($customize_points!=0){
					$new_point_value=$customer_info->points-$customize_points; 
				}
				// });
				// echo $new_point_value; exit;
				// edited by HeinHtetAung at Janu 31 For Member Points Fix });
				//echo $customer_info->current_spend_for_points; exit;
		...
		foreach($items as $line=>$item)
		{
			if (isset($item['item_id']))
			{
				...
				{
					//echo "ei";
					$ori_cost_price=$item['cost_price'];;
					...
					$fifoinfos=array(); //Edited by HeinHtetAung @18Feb for In Details Sales Report, Add Gross Sales Amount, Discount Amount & COGS Amount 
					//echo $has_fifo;
					if($has_fifo==1){
						//echo "ei1";
						...
						if($query->num_rows() > 0)
						{
							...
							foreach($rows as $row){
								...
								if($oldqty>=$buyqty){
									...
									array_push($fifoinfos, array('id'=>$fifo_id, 'qty'=>$buyqty, 'cost'=>$oldcost)); 									
									$buyqty=0;
									break;
									...
									array_push($fifoinfos, array('id'=>$fifo_id, 'qty'=>$oldqty, 'cost'=>$oldcost)); 
							...
							if($buyqty>0){
								...
								array_push($fifoinfos, array('id'=>0, 'qty'=>$buyqty, 'cost'=>$ori_cost_price)); //Edited by HeinHtetAung @18Feb for In Details Sales Report, Add Gross Sales Amount, Discount Amount & COGS Amount 
							}
				...
				$fifoinfos=json_encode($fifoinfos); // //Edited by HeinHtetAung @18Feb for In Details Sales Report, Add Gross Sales Amount, Discount Amount & COGS Amount 
				...
				(
					...
					'discount_type'=>(isset($item['discount_type']))? $item['discount_type'] : "%", // DiscountAmountChange
					'discount_amount'=>(isset($item['discount_amount']))? $item['discount_amount'] : 0, // DiscountAmountChange
					...
					'fifoinfos' => $fifoinfos //Edited by HeinHtetAung @18Feb for In Details Sales Report, Add Gross Sales Amount, Discount Amount & COGS Amount 
				);
				...
				if ($suspended != 2)
				{
					...
					// Edited by HeinHtetAung to get multilocation selling ({
					if (!$cur_item_info->is_service)
					{
						$selected_location_id=(isset($item['location_id']))? $item['location_id'] : "";
						$cur_item_location_info->quantity = $cur_item_location_info->quantity !== NULL ? $cur_item_location_info->quantity : 0;
						
						if($selected_location_id!=""){
							$this->Item_location->save_quantity($item['location_qty'] - $item['quantity'], $item['item_id'], $selected_location_id);
						}else{
					...
					{			
						...
						$this->email->from($this->Location->get_info_for_key('email') ? $this->Location->get_info_for_key('email') : 'no-reply@mg.apps.amztechnology.com/netstartposv2', $this->config->item('company'));
	...
	function _create_sales_items_temp_table_query($where)
	{
		...
		(SELECT IF(".$this->db->dbprefix('items').'.name="'.lang('common_discount').'"'.',0,quantity_purchased) as actual_quantity_purchased,'.$this->db->dbprefix('sales').".location_id as location_id, ".$this->db->dbprefix('sales').".deleted as deleted,".$this->db->dbprefix('sales').".deleted_by as deleted_by, sale_time, date(sale_time) as sale_date, ".$this->db->dbprefix('registers').'.name as register_name,'.$this->db->dbprefix('sales_items').".sale_id, comment,payment_type, customer_id, employee_id, sold_by_employee_id, 
		...
		*(SUM(CASE WHEN cumulative = 1 THEN percent ELSE 0 END))/100) as tax,
		...
		UNION ALL
		(SELECT quantity_purchased as actual_quantity_purchased,".$this->db->dbprefix('sales').".location_id as location_id, ".$this->db->dbprefix('sales').".deleted as deleted,".$this->db->dbprefix('sales').".deleted_by as deleted_by, sale_time, date(sale_time) as sale_date, ".$this->db->dbprefix('registers').'.name as register_name,'.$this->db->dbprefix('sales_item_kits').".sale_id, comment,payment_type, customer_id, employee_id, sold_by_employee_id,
	...
	function get_sales_amount_for_range($start_date, $end_date)
	{
		...
		$this->Sale->create_sales_items_temp_table(array('start_date'=>$start_date, 'end_date'=>$end_date, 'sale_type' => 'sales'));
	...
	function can_void_cc_sale($sale_id)
	{
		...
		foreach($result as $row)
		{
			if ($processor == 'mercury' || $processor == 'heartland' || $processor = 'evo')
			{
